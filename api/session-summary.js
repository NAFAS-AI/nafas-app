...
async function supabaseFetch(path, method, body) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[SESSION] Supabase error:', res.status);
      return null;
    }
...
أجيبي بـ JSON فقط بالشكل التالي:
{
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[SESSION] Gemini API error:', res.status);
      return null;
    }
    const data = await res.json();

    // Check for MAX_TOKENS or safety blocks
    const finishReason = data.candidates?.[0]?.finishReason;
    if (finishReason === 'SAFETY') {
      console.error('[SESSION] Gemini blocked by safety filter');
      return null;
    }

    const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!text) {
      console.error('[SESSION] Gemini returned no text. Finish reason:', finishReason);
      return null;
    }
